...
//    q_vf  =  pba->vf_parameters[5];           // Do not comment if you want to give values for q            
    ...
    beta_vf = pba->vf_parameters[3];      // beta = 1 (Nakamura case), beta = 0 (Noemi case)
  ...
  /*  cdm */
    ...
    if (pba->has_cdm_vf == _FALSE_){
      ...
      /* Auxiliar variable r_{Q} = \frac{Qf_{,X}a^{-3}}{G_{2,X}} */
...
/** Pressure of the vector field. */
  ...
  /*  Add P_vf_prime here */
  /* We need to add P'_{DE}. Then, we computed w_{vf}' and rho_vf' and using */
 ...
 /*h_vf = H'/aH^{2} = \dot{H}/H^{2} is a dimensionless and auxiliary variable to write \Omega_{i}' */   
...
int background_solve(
  ...
  /* conformal distance in Mpc (equal to c*pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3);*pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3);omoving radius in flat case) */
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_vf == _TRUE_) {
      ...
      printf(" -> Vector field parameters: [s, b2, P2, q, Q, q, rho_vf_ini] = \n");
      printf("                    [");
      for (index_vf=0; index_vf<pba->vf_parameters_size-1; index_vf++) {
        printf("%.e, ",pba->vf_parameters[index_vf]);
      }
     printf("%.e]\n",pow(10.0,-pba->vf_parameters[6])*pow(ppr->a_ini_over_a_today_default,4.*pba->vf_parameters[0]));
    } 
...
int background_initial_conditions(
 ...
 if (pba->has_vf == _TRUE_) {        
    pvecback_integration[pba->index_bi_rho_vf] = pow(10.0,-pba->vf_parameters[6])*pow(a,4.*pba->vf_parameters[0]); 
...
//    q_vf  =  pba->vf_parameters[5];           // Do not comment if you want to give values for q        
    ...
    /* We need to write again \rho_{r} and \rho_{m} to define \Omega_{i} */    rho_R = pvecback[pba->index_bg_rho_g];
    if (pba->has_ur == _TRUE_) {
      rho_R += pvecback[pba->index_bg_rho_ur];
    }
...
/* Auxiliar varianle \eps_{phi} = \rho_{DE}' /aH\rho_{DE} */
  ...
  rho_M = pvecback[pba->index_bg_rho_b];
  if (pba->has_cdm == _TRUE_ && pba->has_cdm_vf  == _FALSE_) {
    rho_M += pvecback[pba->index_bg_rho_cdm];
  }
  else if (pba->has_cdm_vf == _TRUE_){
    rho_M += pvecback[pba->index_bg_rho_cdm];
  }
